<template>
    <div v-if="loading === false">
        <div v-if="otherChargeAdd === true">
            <h1 class="pt-5">
                Add Other Charges
            </h1>
            <div class="row">
                <div class="col-lg-10 m-auto">
                    <form @submit.prevent="addOtherCharge" @keydown="form.onKeydown($event)">
                        <!-- Name -->
                        <div class="mb-3 row">
...
export default {
    ...
    async mounted() {
        this.$useHead({
            title: 'Other Charge',
            description: 'Other Charge create page'
        });

        if (this.authStore.user === null) {
            this.loading = true
        } else {
            const permissionsToCheck = ['otherChargeAdd'];
            const permissionResults = await checkPermissions(permissionsToCheck);
            permissionsToCheck.forEach(permission => {
                if (permissionResults[permission]) {
                    this[permission] = true;
                    if (permission === 'otherChargeAdd') {
                        this.loading = false;
                    }
                }
            });
    ...
    methods: {
        async addOtherCharge() {
            try {
                await this.form.post('/api/othercharge')
                    .then(response => {
