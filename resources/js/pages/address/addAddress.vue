<template>
    <div v-if="loading === false">
        <div v-if="addressAdd === true">
            <h1 class="pt-5">
                Add Address
            </h1>
            <div class="row">
                <div class="col-lg-10 m-auto">
                    <form @submit.prevent="addAddress" @keydown="form.onKeydown($event)">
...
export default {
    ...
    async mounted() {
        ...
        if (this.authStore.user === null) {
            ...
            const permissionsToCheck = ['addressAdd'];
            const permissionResults = await checkPermissions(permissionsToCheck);
            permissionsToCheck.forEach(permission => {
                if (permissionResults[permission]) {
                    this[permission] = true;
                    if (permission === 'addressAdd') {
                        this.loading = false;
                    }
                }
            });
    ...
    methods: {
        ...
        async addAddress() {
            try {
                await this.form.post('/api/address')
                    .then(response => {
        ...
        async fetchCreateData() {
            try {
                const response = await this.$axios.get(`/api/address/create`); // Replace with your API endpoint
