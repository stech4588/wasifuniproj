<template>
    <div v-if="loading === false">
        <div v-if="reviewView === true">
            <h1>Review</h1>

            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-md-8">
                        <form @submit.prevent="SearchReview" @keydown="form.onKeydown($event)">
...
<!--                <router-link v-if="reviewAdd" :to="{ name: 'productUnit.add' }" class="btn linked-icon">-->
                ...
                <table class="table table-bordered ">
                    ...
                    <tr v-for="(item, index) in currentPageData" :key="index">
                        ...
                        <td class="text-center">
                            <router-link :to="{ path: `/review/${item.id}` }" class="btn linked-icon">
                                <font-awesome-icon icon="eye" fixed-width />
                            </router-link>
                            <router-link v-if="reviewUpdate " :to="{ path: `/review/${item.id}/edit` }" class="btn linked-icon">
                                <font-awesome-icon icon="pen" fixed-width />
                            </router-link>
                            <button v-if="reviewDelete " class="btn linked-icon" @click="deleteReview(item.id)">
...
export default {
    ...
    async mounted() {
        ...
        if (this.authStore.user === null) {
            ...
            const permissionsToCheck = ['reviewAdd', 'reviewUpdate', 'reviewView', 'reviewDelete'];
            const permissionResults = await checkPermissions( permissionsToCheck);
            permissionsToCheck.forEach(permission => {
                if (permissionResults[permission]) {
                    this[permission] = true;
                    if (permission === 'reviewView') {
                        this.loading = false;
                    }
                }
            });
    ...
    methods: {
        async changePage (page) {
            const response = await this.$axios.get(`api/review?page=${page}&view=${this.view}&search=${this.form.searchInput}`)
        ...
        async deleteReview(id) {
            const permission = await this.showConfirmationDialog('Are you sure you want to delete this review?')
            if (permission) {
                try {
                    await this.$axios
