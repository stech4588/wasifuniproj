    ...
    <div v-if="loading === false">
        <div v-if="productView === true">
            ...
            <div class="text-end my-3">
                <router-link v-if="productAdd" :to="{ name: 'product.add' }" class="btn linked-icon">
                    ...
                    <tr v-for="(item, index) in currentPageData" :key="index">
                        ...
                        <td class="text-center">
                            <div>
                                ...
                                <button v-if="!item.is_active && canApproved" class="btn linked-icon" @click="approveProduct(item.id)">
                                    Approve
                                </button>

                                <!-- Show view, edit, and delete buttons for active products -->
                                <router-link v-else :to="{ path: `/product/${item.id}` }" class="btn linked-icon">
                                    <font-awesome-icon icon="eye" fixed-width />
                                </router-link>
                                <router-link v-if="productUpdate && item.is_active" :to="{ path: `/product/${item.id}/edit` }" class="btn linked-icon">
                                    <font-awesome-icon icon="pen" fixed-width />
                                </router-link>
                                <button v-if="productDelete && item.is_active" class="btn linked-icon" @click="deleteProduct(item.id)">
...
export default {
    ...
    async mounted() {
        ...
        if (this.authStore.user === null) {
            this.loading = true;
             ...
             ;
            const permissionsToCheck = ['productAdd', 'productUpdate', 'productView', 'productDelete', 'canApproved'];
    ...
    methods: {
        async changePage (page) {
            const response = await this.$axios.get(`api/product?page=${page}&view=${this.view}&search=${this.form.searchInput}`)
        ...
        async deleteProduct(id) {
            const permission = await this.showConfirmationDialog('Are you sure you want to delete this Product?')
            if (permission) {
                try {
                    await this.$axios
