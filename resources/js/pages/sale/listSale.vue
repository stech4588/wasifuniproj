        ...
        <div v-if="saleView === true">
            ...
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-md-8">
                        <form @submit.prevent="SearchSale" @keydown="form.onKeydown($event)">
            ...
            <div class="text-end my-3">
                <router-link v-if="saleAdd" :to="{ name: 'sale.add' }" class="btn linked-icon">
                ...
                <table class="table table-bordered ">
                    ...
                    <tr v-for="(item, index) in currentPageData" :key="index">
                        ...
                        <td class="text-center">
                            <router-link :to="{ path: `/sale/${item.id}` }" class="btn linked-icon">
                                <font-awesome-icon icon="eye" fixed-width />
                            </router-link>
                            <router-link v-if="saleUpdate " :to="{ path: `/sale/${item.id}/edit` }" class="btn linked-icon">
                                <font-awesome-icon icon="pen" fixed-width />
                            </router-link>
                            <button v-if="saleDelete " class="btn linked-icon" @click="deleteSale(item.id)">
...
export default {
    ...
    async mounted() {
        ...
        await this.fetchSaleList();
    },

    methods: {
        async changePage (page) {
            const response = await this.$axios.get(`api/sale?page=${page}&view=${this.view}&search=${this.form.searchInput}`)
