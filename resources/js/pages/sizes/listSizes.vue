<template>
    <div v-if="loading === false">
        <div v-if="sizesView === true">
            <h1>Category Sizes</h1>

            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-md-8">
                        <form @submit.prevent="SearchSizes" @keydown="form.onKeydown($event)">
            ...
            <div class="text-end my-3">
                <router-link v-if="sizesAdd" :to="{ name: 'sizes.add' }" class="btn linked-icon">
                ...
                <table class="table table-bordered ">
                    ...
                    <tr v-for="(item, index) in currentPageData" :key="index">
                        ...
                        <td class="text-center">
                            <router-link :to="{ path: `/viewSizes/${item.id}` }" class="btn linked-icon">
                                <font-awesome-icon icon="eye" fixed-width />
                            </router-link>
                            <router-link v-if="sizesUpdate " :to="{ path: `/updateSizes/${item.id}` }" class="btn linked-icon">
                                <font-awesome-icon icon="pen" fixed-width />
                            </router-link>
                            <button v-if="sizesDelete " class="btn linked-icon" @click="deleteSizes(item.id)">
...
export default {
    ...
    async mounted() {
        ...
        await this.fetchSizesList();
    },

    methods: {
        async changePage (page) {
            const response = await this.$axios.get(`api/sizes?page=${page}&view=${this.view}&search=${this.form.searchInput}`)
            this.currentPage = response.data.data.current_page
            this.currentPageData = response.data.data.data
        },

        async fetchSizesList() {
            try {
                await this.$axios
                    .get(`/api/sizes?view=${this.view}`)
                    .then(response => {
                        this.currentPageData = response.data.data.data
        ...
        async deleteSizes(id) {
            const permission = await this.showConfirmationDialog('Are you sure you want to delete this Size?')
            if (permission) {
                try {
                    await this.$axios
                        .delete(`/api/sizes/${id}`)
