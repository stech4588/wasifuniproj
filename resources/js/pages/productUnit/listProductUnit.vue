        ...
        <div v-if="productUnitView === true">
            ...
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-md-8">
                        <form @submit.prevent="SearchProductUnit" @keydown="form.onKeydown($event)">
            ...
            <div class="text-end my-3">
                <router-link v-if="productUnitAdd" :to="{ name: 'productUnit.add' }" class="btn linked-icon">
                ...
                <table class="table table-bordered ">
                    ...
                    <tr v-for="(item, index) in currentPageData" :key="index">
                        ...
                        <td class="text-center">
                            <router-link :to="{ path: `/productUnit/${item.id}` }" class="btn linked-icon">
                                <font-awesome-icon icon="eye" fixed-width />
                            </router-link>
                            <router-link v-if="productUnitUpdate " :to="{ path: `/productUnit/${item.id}/edit` }" class="btn linked-icon">
                                <font-awesome-icon icon="pen" fixed-width />
                            </router-link>
                            <button v-if="productUnitDelete " class="btn linked-icon" @click="deleteProductUnit(item.id)">
...
export default {
    ...
    methods: {
        async changePage (page) {
            const response = await this.$axios.get(`api/productunit?page=${page}&view=${this.view}&search=${this.form.searchInput}`)
            this.currentPage = response.data.data.current_page
            this.currentPageData = response.data.data.data
        },

        async fetchProductUnitList() {
            try {
                await this.$axios
                    .get(`/api/productunit?view=${this.view}`)
                    .then(response => {
                        this.currentPageData = response.data.data.data
        ...
        async deleteProductUnit(id) {
            const permission = await this.showConfirmationDialog('Are you sure you want to delete this Product Unit?')
            if (permission) {
                try {
                    await this.$axios
                        .delete(`/api/productunit/${id}`)
                        .then(response => {
