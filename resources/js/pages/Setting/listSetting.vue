    ...
    <div v-if="loading === false">
        <div v-if="settingView === true">
            ...
            <div class="text-end my-3">
                <router-link v-if="settingAdd" :to="{ name: 'setting.add' }" class="btn linked-icon">
                ...
                <table class="table table-bordered ">
                    ...
                    <tr v-for="(item, index) in currentPageData" :key="index">
                        ...
                        <td class="text-center">
                            <router-link :to="{ path: `/setting/${item.id}` }" class="btn linked-icon">
                                <font-awesome-icon icon="eye" fixed-width />
                            </router-link>
                            <router-link v-if="settingUpdate " :to="{ path: `/setting/${item.id}/edit` }" class="btn linked-icon">
                                <font-awesome-icon icon="pen" fixed-width />
                            </router-link>
                            <button v-if="settingDelete " class="btn linked-icon" @click="deleteSetting(item.id)">
...
export default {
    ...
    methods: {
        async changePage (page) {
            const response = await this.$axios.get(`api/setting?page=${page}&view=${this.view}&search=${this.form.searchInput}`)
        ...
        async fetchSettingList() {
            try {
                await this.$axios
                    .get(`/api/setting?view=${this.view}`)
                    .then(response => {
                        this.currentPageData = response.data.data.data
        ...
        async deleteSetting(id) {
            const permission = await this.showConfirmationDialog('Are you sure you want to delete this Setting?')
            if (permission) {
                try {
                    await this.$axios
